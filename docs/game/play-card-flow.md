...
**重要说明 - 语音通道独立**:
- 报牌语音使用 `ChannelType.ANNOUNCEMENT` 通道（值为 8）
- 聊天语音使用 `ChannelType.PLAYER_0` 到 `ChannelType.PLAYER_7` 通道（值为 0-7）
- `playNextTurn` 只等待报牌通道，不等待聊天通道
- 这样可以确保聊天语音不会阻塞游戏流程，游戏速度更快
