# 团队计分规则与聊天系统重新设计

## 📋 改动概述

1. **计分规则**：从个人分数改为**团队分数**
2. **AI建议系统**：提供出牌选择并**解释原因**
3. **聊天系统**：围绕打牌沟通，支持**对骂、互相指责**等

---

## 🎯 改动1：团队计分系统

### 当前系统（个人计分）

```
玩家1: +150分
玩家2: +80分
玩家3: -100分
玩家4: -130分
```

### 新系统（团队计分）

#### 方案1：完全团队计分 ⭐⭐⭐⭐⭐
**描述**：只显示团队总分数，不显示个人分数

```
团队A（玩家1 + 玩家3）: +50分
团队B（玩家2 + 玩家4）: -50分
```

**计分逻辑**：
```typescript
// 团队总分数 = 所有队员的分数之和
teamScore = player1Score + player2Score;

// 游戏结束时
if (teamScore > 0) {
  // 团队获胜
  team1.result = 'win';
  team2.result = 'lose';
}
```

#### 方案2：团队分数 + 个人贡献 ⭐⭐⭐⭐
**描述**：显示团队分数，但保留个人贡献数据（用于分析）

```
团队A总分: +50分
  - 玩家1贡献: +80分
  - 玩家3贡献: -30分

团队B总分: -50分
  - 玩家2贡献: +20分
  - 玩家4贡献: -70分
```

**优点**：可以看到每个队友的贡献，帮助改进策略

#### 方案3：团队分数 + 个人排名 ⭐⭐⭐
**描述**：团队分数决定胜负，但保留个人排名（用于奖励）

```
团队A: +50分（获胜）
  - 玩家1: 第1名（出完牌顺序）
  - 玩家3: 第3名

团队B: -50分（失败）
  - 玩家2: 第2名
  - 玩家4: 第4名
```

### 推荐方案：方案1（完全团队计分）

**理由**：
- ✅ 简化计分逻辑
- ✅ 强化团队合作意识
- ✅ 符合合作模式的设计理念

### 计分规则修改点

#### 1. 轮次分数分配

**原规则**：
```
玩家1出牌拿到15分 → 玩家1 +15分
```

**新规则**：
```
玩家1出牌拿到15分 → 团队A +15分（不分配给个人）
```

#### 2. 墩的计分

**原规则**：
```
玩家1出墩 → 从其他玩家扣除 → 玩家1获得分数
```

**新规则**：
```
玩家1出墩 → 从对方团队扣除 → 团队A获得分数
```

#### 3. 最终规则

**原规则**：
```
第一名 +30分
最后一名 -30分
```

**新规则**：
```
头游团队 +30分（给团队总分）
末游团队 -30分（从团队总分扣除）

或者：
获胜团队 +60分（简单粗暴）
失败团队 -60分
```

#### 4. 末游剩余分牌

**原规则**：
```
最后一名未出的分牌 → 扣除自己分数 → 给第二名
```

**新规则**：
```
末游团队未出的分牌 → 从团队总分扣除 → 给头游团队
```

### 实现要点

```typescript
interface TeamScore {
  teamId: number;
  players: number[];  // 队员ID
  teamScore: number;  // 团队总分数
  roundScore: number; // 当前轮次团队分数
}

// 分数分配逻辑
function allocateRoundScore(
  playerIndex: number,
  score: number,
  teams: Team[]
): void {
  const team = findTeamByPlayer(playerIndex, teams);
  team.teamScore += score;  // 直接加到团队分数
  // 不分配给个人
}
```

### UI显示变化

**原UI**：
```
玩家1: 150分
玩家2: 80分
玩家3: -100分
玩家4: -130分
```

**新UI**：
```
╔═══════════════════════════╗
║  团队A（你 + AI队友）     ║
║  🎯 团队分数: +50分       ║
║  📊 当前轮次: +15分       ║
╚═══════════════════════════╝

╔═══════════════════════════╗
║  团队B（对手1 + 对手2）   ║
║  🎯 团队分数: -50分       ║
║  📊 当前轮次: 0分         ║
╚═══════════════════════════╝
```

---

## 🤖 改动2：AI建议系统增强

### 当前AI建议系统

**现状**：
- 提供出牌建议
- 简单提示"AI建议：要不起"

### 新系统设计

#### 核心需求
1. ✅ 提供多个出牌选择
2. ✅ 解释每个选择的原因
3. ✅ 评估每个选择的收益
4. ✅ 帮助玩家理解策略

### 设计方案

#### 方案1：多选择对比 ⭐⭐⭐⭐⭐

**界面设计**：
```
┌─────────────────────────────────────────┐
│ 💡 AI策略建议                           │
├─────────────────────────────────────────┤
│                                         │
│ 推荐选择1：主动要不起                   │
│ ⭐⭐⭐⭐⭐                              │
│ 📊 预期团队收益: +25分                 │
│ 💭 原因：                              │
│   • 你的队友手牌更少（8张），更适合出牌│
│   • 当前轮次有15分，队友可以保护分牌   │
│   • 你保留大牌（A、A、A）用于关键轮次  │
│   • 团队优势: +10分                    │
│                                         │
│ [选择此建议]                            │
│                                         │
├─────────────────────────────────────────┤
│                                         │
│ 选择2：出K（单张）                      │
│ ⭐⭐⭐                                │
│ 📊 预期团队收益: +15分                 │
│ 💭 原因：                              │
│   • 你可以立即拿到当前轮次的15分       │
│   • 风险：暴露了你手上有大牌           │
│   • 可能影响后续策略                   │
│   • 团队优势: +5分                     │
│                                         │
│ [选择此建议]                            │
│                                         │
├─────────────────────────────────────────┤
│                                         │
│ 选择3：出A（单张）                      │
│ ⭐⭐                                   │
│ 📊 预期团队收益: +10分                 │
│ 💭 原因：                              │
│   • 可以压过对手                        │
│   • ⚠️ 但浪费了大牌，拆散了潜在炸弹    │
│   • 不推荐：对团队长期收益不利         │
│                                         │
│ [选择此建议]                            │
│                                         │
└─────────────────────────────────────────┘
```

#### 方案2：简化版（推荐用于快速决策）⭐⭐⭐⭐

```
┌─────────────────────────────────────────┐
│ 💡 AI建议：主动要不起                   │
│                                         │
│ 📊 团队收益预期: +25分                 │
│                                         │
│ 💭 原因：                              │
│ • 队友手牌更少，更适合出牌              │
│ • 保护你的大牌（A、A、A）              │
│ • 让队友保护分牌                       │
│                                         │
│ [确认要不起] [查看其他选择]            │
└─────────────────────────────────────────┘
```

### AI建议的内容结构

```typescript
interface AIAdvice {
  recommendedAction: PlayAction;  // 推荐动作
  alternatives: AlternativeAction[];  // 备选动作
  explanation: {
    reason: string;  // 主要原因
    teamBenefit: number;  // 团队收益
    riskLevel: 'low' | 'medium' | 'high';  // 风险等级
    details: string[];  // 详细说明（列表）
  };
}

interface AlternativeAction {
  action: PlayAction;
  rating: number;  // 1-5星
  teamBenefit: number;
  reason: string;
  pros: string[];  // 优点
  cons: string[];  // 缺点
}
```

### 建议生成逻辑

#### 1. 分析当前局势

```typescript
function analyzeSituation(state: GameState): SituationAnalysis {
  return {
    currentRoundScore: 15,  // 当前轮次有15分
    teammateHandCount: 8,   // 队友手牌更少
    teammateHasScoreCards: true,  // 队友有分牌
    myBigCards: ['A', 'A', 'A'],  // 我有大牌
    opponentThreat: 'medium'  // 对手威胁程度
  };
}
```

#### 2. 评估不同选择

```typescript
function evaluateOptions(
  situation: SituationAnalysis,
  availableActions: PlayAction[]
): AIAdvice {
  const options = availableActions.map(action => {
    // 使用MCTS模拟评估
    const simulationResult = mctsSimulate(action, situation);
    
    return {
      action,
      teamBenefit: simulationResult.expectedTeamScore,
      riskLevel: simulationResult.riskLevel,
      reason: generateReason(action, simulationResult)
    };
  });
  
  // 排序，选择最优的
  options.sort((a, b) => b.teamBenefit - a.teamBenefit);
  
  return {
    recommendedAction: options[0],
    alternatives: options.slice(1, 3),  // 提供2个备选
    explanation: generateExplanation(options[0])
  };
}
```

#### 3. 生成解释文本

```typescript
function generateExplanation(option: EvaluatedAction): Explanation {
  const reasons: string[] = [];
  
  // 根据分析结果生成原因
  if (option.teamBenefit > 20) {
    reasons.push(`预期团队收益: +${option.teamBenefit}分`);
  }
  
  if (situation.teammateHandCount < situation.myHandCount) {
    reasons.push(`队友手牌更少（${situation.teammateHandCount}张），更适合出牌`);
  }
  
  if (situation.teammateHasScoreCards) {
    reasons.push(`队友有分牌，需要保护`);
  }
  
  if (option.action.type === 'pass' && hasBigCards()) {
    reasons.push(`保留大牌用于关键轮次`);
  }
  
  return {
    reason: reasons[0],  // 主要原因
    details: reasons     // 详细列表
  };
}
```

### UI交互设计

#### 快速建议模式（默认）
- 显示1个推荐建议
- 点击展开查看原因
- 一键确认

#### 详细对比模式
- 显示3个选择
- 对比优缺点
- 帮助玩家学习策略

---

## 💬 改动3：聊天系统重新设计

### 当前聊天系统

**现状**：
- AI生成对话
- 可能不够贴合牌桌场景
- 缺少针对性的沟通

### 新系统设计目标

1. ✅ **围绕打牌沟通**
2. ✅ **对骂、互相指责**
3. ✅ **策略沟通**（合作模式）
4. ✅ **情绪表达**

### 聊天内容分类

#### 1. 打牌相关沟通 ⭐⭐⭐⭐⭐

##### 出牌时
```
- "跟一手！"
- "压死你！"
- "看我怎么收拾你！"
- "这把我要了！"
```

##### 要不起时
```
- "要不起..."
- "这牌我吃不起"
- "让你们先玩"
- "我看看你们怎么打"
```

##### 出好牌时
```
- "哈哈，炸弹！"
- "我的墩，你们都要不起！"
- "这分我拿了！"
```

##### 分牌争夺
```
- "这15分我要定了！"
- "休想抢我的分！"
- "分牌留给我！"
```

#### 2. 对骂、互相指责 ⭐⭐⭐⭐⭐

##### 攻击性对话
```
- "你打得什么垃圾牌！"
- "会不会玩啊？"
- "这么烂的牌也敢出？"
- "你这是在送分！"
```

##### 挑衅
```
- "就这？"
- "还有没有更好的？"
- "你的大牌呢？"
- "不服再来！"
```

##### 嘲讽
```
- "哈哈，你的牌不行啊"
- "早就看出你要不起"
- "你的策略太明显了"
- "这就是你的水平？"
```

##### 指责（针对错误决策）
```
- "你刚才不应该出那张牌"
- "你害得我们丢了15分"
- "你的策略有问题"
- "你应该配合我"
```

#### 3. 策略沟通（合作模式）⭐⭐⭐⭐⭐

##### 配合沟通
```
- "我来！"
- "你出，我保护"
- "保留大牌"
- "分牌留给我"
- "让我来拿这分"
```

##### 信号沟通
```
- "我有大牌"
- "你先出"
- "保护分牌"
- "快速出完"
```

#### 4. 情绪表达 ⭐⭐⭐⭐

##### 兴奋
```
- "太好了！"
- "这把稳了！"
- "我们赢了！"
```

##### 沮丧
```
- "这牌太烂了"
- "运气不好"
- "又被抢分了"
```

##### 紧张
```
- "小心点"
- "这很关键"
- "不能出错"
```

### 聊天触发机制

#### 基于游戏事件触发

```typescript
interface ChatTrigger {
  event: GameEvent;
  context: GameContext;
  generateChat(): ChatMessage[];
}

// 事件类型
enum GameEvent {
  PLAYER_PLAY_CARDS,      // 玩家出牌
  PLAYER_PASS,            // 玩家要不起
  BOMB_PLAYED,            // 出炸弹
  SCORE_CARD_PLAYED,      // 出分牌
  ROUND_END,              // 轮次结束
  MISTAKE_MADE,           // 错误决策
  GOOD_PLAY,              // 好牌
  TEAMMATE_NEEDS_HELP,    // 队友需要帮助
}
```

#### 具体触发逻辑

##### 1. 出牌时触发
```typescript
onPlayerPlayCards(playerId, cards, context) {
  // 生成相关聊天
  if (isBomb(cards)) {
    generateChat("bomb_celebration", context);
  }
  
  if (hasScoreCards(cards)) {
    generateChat("score_card_claim", context);
  }
  
  // 对手的反应
  opponents.forEach(opponent => {
    if (canBeat(cards, opponent.hand)) {
      generateChat("threat_response", opponent, context);
    }
  });
}
```

##### 2. 错误决策触发
```typescript
onMistakeDetected(playerId, mistake, context) {
  // 队友的反应
  if (isTeammate(playerId)) {
    generateChat("teammate_criticism", context);
  } else {
    generateChat("opponent_taunt", context);
  }
}
```

##### 3. 分牌争夺触发
```typescript
onScoreCardPlayed(roundScore, context) {
  // 所有玩家都想争夺
  players.forEach(player => {
    if (canTakeScore(player)) {
      generateChat("score_competition", player, context);
    }
  });
}
```

### 聊天生成策略

#### 1. 基于角色性格

```typescript
interface CharacterPersonality {
  aggressive: number;      // 攻击性 0-1
  cooperative: number;     // 合作性 0-1
  taunting: number;        // 嘲讽倾向 0-1
  supportive: number;      // 支持性 0-1
}

// 不同性格生成不同的聊天内容
function generateChatByPersonality(
  event: GameEvent,
  personality: CharacterPersonality
): string {
  if (personality.aggressive > 0.7) {
    return generateAggressiveChat(event);
  } else if (personality.cooperative > 0.7) {
    return generateCooperativeChat(event);
  }
  // ...
}
```

#### 2. 基于游戏状态

```typescript
function generateContextualChat(
  event: GameEvent,
  gameState: GameState
): string {
  // 根据当前分数差距
  if (gameState.teamScoreDiff > 50) {
    return generateLeadingChat(event);  // 领先时的对话
  } else if (gameState.teamScoreDiff < -50) {
    return generateLosingChat(event);   // 落后时的对话
  }
  
  // 根据手牌情况
  if (player.handCount < 5) {
    return generateEndgameChat(event);  // 接近结束
  }
  
  // ...
}
```

#### 3. 基于关系（合作模式）

```typescript
function generateTeamChat(
  event: GameEvent,
  relationship: 'teammate' | 'opponent'
): string {
  if (relationship === 'teammate') {
    // 队友之间的对话
    return generateCooperativeChat(event);
  } else {
    // 对手之间的对话
    return generateCompetitiveChat(event);
  }
}
```

### 聊天内容示例库

#### 出牌相关
```typescript
const playChats = {
  good_play: [
    "这把我要了！",
    "看我的！",
    "压死你！",
    "这分我拿了！"
  ],
  bomb: [
    "炸弹！哈哈！",
    "我的炸弹来了！",
    "你们都要不起！",
    "看我炸死你们！"
  ],
  pass: [
    "要不起...",
    "这牌我吃不起",
    "让你们先玩",
    "我看看"
  ]
};
```

#### 对骂相关
```typescript
const tauntChats = {
  opponent_bad_play: [
    "你打得什么垃圾牌！",
    "会不会玩啊？",
    "这么烂的牌也敢出？"
  ],
  opponent_pass: [
    "就这？",
    "你的大牌呢？",
    "不服再来！"
  ],
  score_stolen: [
    "想抢我的分？",
    "这分我要定了！",
    "休想！"
  ]
};
```

#### 合作相关
```typescript
const cooperationChats = {
  teammate_good: [
    "好牌！",
    "漂亮！",
    "配合得好！"
  ],
  teammate_mistake: [
    "你刚才不应该出那张",
    "保留大牌啊",
    "让我来"
  ],
  coordination: [
    "我来！",
    "你出，我保护",
    "分牌留给我"
  ]
};
```

### UI设计

#### 聊天气泡样式

```
┌─────────────────────────┐
│ 👤 玩家1                │
│ "看我的炸弹！"          │
│ ⏰ 2秒前                │
└─────────────────────────┘

┌─────────────────────────┐
│ 🤖 AI队友               │
│ "好牌！我来配合你"      │
│ ⏰ 1秒前                │
└─────────────────────────┘

┌─────────────────────────┐
│ 😤 对手1                │
│ "你打得什么垃圾牌！"    │
│ ⏰ 刚刚                 │
└─────────────────────────┘
```

#### 快捷聊天（可选）

```
[快捷回复]
┌──────┬──────┬──────┐
│ 我来 │ 你来 │ 好牌 │
├──────┼──────┼──────┤
│ 要不起│ 保护 │ 抢分 │
└──────┴──────┴──────┘
```

---

## 🔄 系统联动

### 1. 聊天 + 计分联动

```
当团队分数领先时：
- AI会说："我们领先了，保持住！"

当团队分数落后时：
- AI会说："落后了，要加油啊！"

当拿到关键分数时：
- AI会说："这分很关键，我们赢了！"
```

### 2. 聊天 + AI建议联动

```
AI建议时：
- "我建议你主动要不起，因为队友手牌更少"

玩家确认建议后：
- AI队友会说："好，我来出！"
- 对手可能会说："你们在搞什么？"
```

### 3. 聊天 + 游戏事件联动

```
出炸弹时：
- 自动触发："炸弹！"
- 对手反应："你狠！"

分牌争夺时：
- 自动触发："这分我要了！"
- 其他人反应："想得美！"
```

---

## 📊 实现优先级

### 高优先级
1. ✅ **团队计分系统** - 核心改动
2. ✅ **聊天系统重构** - 核心体验
3. ✅ **AI建议增强** - 核心功能

### 中优先级
1. ⚠️ 聊天快捷回复
2. ⚠️ 聊天情绪表达
3. ⚠️ 详细建议对比

### 低优先级
1. 📝 聊天历史记录
2. 📝 聊天统计分析
3. 📝 自定义聊天内容

---

## 💡 总结

这三个改动都非常重要：

1. **团队计分** - 强化团队意识，简化计分逻辑
2. **AI建议增强** - 帮助玩家理解策略，提升游戏体验
3. **聊天系统重构** - 增加游戏趣味性和真实感

它们共同构成了一个**完整的合作模式体验**！

