...
export class MasterAIBrain {
  ...
  async handleTurn(playerId: number, gameState: GameState): Promise<AIBehavior> {
    ...
    try {
      ...
      // 临时关闭AI决策后的聊天生成（调试用）
      console.log('[MasterAIBrain] AI决策后的聊天生成已临时关闭（调试用）');
      /*
      ...
      (async () => {
        try {
          ...
          console.error('[MasterAIBrain] 异步生成消息失败:', error);
        }
      })();
      */
  ...
  async handleUserMessage(playerId: number, content: string, gameState?: GameState): Promise<void> {
    ...
    console.log(`[MasterAIBrain] 收到来自玩家${playerId}的消息: ${content}`);
    ...
    this.commScheduler.generateBatchMessages(
      ...
      if (messages.size > 0) {
        console.log(`[MasterAIBrain] 生成了${messages.size}条针对用户消息的回应`);
      }
    }).catch(err => {
      console.error('[MasterAIBrain] 生成用户消息回应失败:', err);
    });
  }
  ...
  updateLLMConfig(updates: {
    ...
    if (!this.llmService) {
      console.warn('[MasterAIBrain] LLM服务未启用，无法更新配置');
      return;
    }
    ...
    console.log('[MasterAIBrain] LLM配置已动态更新', updates);
  }
