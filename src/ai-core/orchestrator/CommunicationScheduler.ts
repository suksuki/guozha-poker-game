...
const MAX_HISTORY_LENGTH = 10; // Context Pruning: Keep last 10 interactions
...
export class CommunicationScheduler {
  ...
  // Delta Updates / Context History
  private sessionHistories: Map<number, ChatMessage[]> = new Map();
  // Store the last known game round to detect round changes for history clearing
  ...
  constructor(
    ...
    // Listen for state changes to update history
    // Note: This relies on someone calling updateHistory explicitly or binding events
    // For now we assume the caller (MasterAIBrain) might not call updateHistory,
    // so we can also hook into generateMessageContent to perform lazy updates if needed,
    // but proactive updates are better for Delta logic.
  }
  ...
  async initialize(): Promise<void> {
    ...
    // Clear histories on init
    this.sessionHistories.clear();
  }

  /**
   * Delta Update: Record a game event into the history of all/specific players
   * External triggers should call this to maintain context.
   */
  handleGameEvent(eventDescription: string, specificPlayerId?: number): void {
    this.updateHistory(eventDescription, specificPlayerId);
  }

  updateHistory(eventDescription: string, specificPlayerId?: number): void {
    const timestamp = new Date().toLocaleTimeString();
    const content = `[${timestamp}] ${eventDescription}`;

    if (specificPlayerId !== undefined) {
      this.addToHistory(specificPlayerId, { role: 'user', content });
    } else {
      // Broadcast to all players
      this.players.forEach((_, playerId) => {
        this.addToHistory(playerId, { role: 'user', content });
      });
    }
  }

  private addToHistory(playerId: number, message: ChatMessage): void {
    if (!this.sessionHistories.has(playerId)) {
      this.sessionHistories.set(playerId, []);
    }
    const history = this.sessionHistories.get(playerId)!;

    // Check for round change to reset history if needed (optional strategy)
    // For now, we just append.
    history.push(message);

    // Context Pruning: Keep last 15 messages (increased from 10 for better context)
    // We rely on this sliding window as the "Context History"
    if (history.length > 15) {
      this.sessionHistories.set(playerId, history.slice(-15));
    }
  }
  ...
  private async generateMessageContent(
    ...
    // 使用LLM生成
    try {
      // 构建 Delta-based Prompt (ChatMessage[])
      ...
      // 🔍 调试：打印实际使用的LLM配置
      console.log('[CommunicationScheduler] 调用LLM', {
        messagesCount: messages.length,
        llmServiceConfig: this.llmService.getConfig ? this.llmService.getConfig() : 'no getConfig method',
        firstMessage: messages[0]?.content.substring(0, 100)
      });

      const response = await this.llmService.call({
        purpose: 'communication',
        prompt: messages, // Pass array instead of string
        priority, // 使用优先级
        options: {
      ...
      // 添加AI的回答到历史中，维持上下文连贯性
      this.addToHistory(playerId, { role: 'assistant', content: content });
      ...
      return {
        ...
        _metadata: {
          rawResponse,
          fullPrompt: JSON.stringify(messages) // Record structured prompt
        }
      };
    } catch (error) {
...
- 如果不想说话，可以输出 "..." 或 "PASS"`;
    ...
    // 2. Insert recent context summary (Snapshot injection)
    // Even with delta updates, a periodic "State Snapshot" helps clarify current status (hand count, scores)
    const snapshot = this.buildGameStateSnapshot(context);
    messages.push({ role: 'user', content: `[当前状态快照]\n${snapshot}` });

    // 3. Append Session History (Recent Events)
    if (this.sessionHistories.has(playerId)) {
  ...
  private getPersonaDescription(preset: string): string {
    const map: Record<string, string> = {
      'aggressive': '激进、挑衅、喜欢嘲讽对手，赢了狂妄输了嘴硬。',
      'conservative': '谨慎、保守、话少，注重稳妥。',
      'balanced': '平和、友善，正常交流。',
      'adaptive': '灵活多变，见人说人话。'
    };
  ...
  /**
   * Deprecated single-string prompt builder (kept for reference or fallback if needed, but not used in new flow)
   */
  private buildCommunicationPrompt(
    ...
    const presetLabel = presetDesc[preset] || '平衡型';
    ...
    // 5. 输出要求
    parts.push(this.buildOutputRequirementSection(presetLabel));
