#!/bin/bash
# 启动APP和Piper TTS服务 (WSL方式)
# 使用方法: ./start-app-with-piper.sh
# 
# 这个脚本与 start-all.sh 功能相同，使用相同的简单可靠的启动方式
...
if check_port 5000; then
    ...
    # 启动Piper TTS服务（后台运行）- 使用原有的start-piper-tts.sh脚本
...
if check_port 3000; then
    ...
    echo "🌐 启动开发服务器（端口3000）..."
    # 启动开发服务器（前台运行，这样可以看到输出）- 使用原有的start.sh脚本
    bash start.sh
fi
