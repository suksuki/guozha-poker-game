...
import { CommunicationMessage, CommunicationContext, CommunicationIntent, Emotion } from '../../types';
...
export class SinglePlayerStrategy implements CommunicationStrategy {
  ...
  constructor(
    ...
    private getPriorityFn: (context: CommunicationContext) => number,
    private determineIntent: (context: CommunicationContext, personality: any) => CommunicationIntent,
    private determineEmotion: (context: CommunicationContext, personality: any) => Emotion
  ) { }
  ...
  async generateSingle(
    ...
    try {
      ...
      const priority = this.getPriorityFn(context);
