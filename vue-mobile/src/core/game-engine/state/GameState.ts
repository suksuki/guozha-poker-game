...
import { GameStatus, Player, Card } from '@/core/types/card';
import { TeamConfig } from '../../types/team';
import { RoundData, RoundDataSnapshot } from '../round/RoundData';

// 重新导出 RoundData 以便其他模块使用
export { RoundData };
export type { RoundDataSnapshot };
...
export interface StateChangeEvent {
  type: 'playerUpdated' | 'statusChanged' | 'roundAdded' | 'roundUpdated' | 'currentPlayerChanged' | 'playerFinished';
  oldState?: any;
  newState?: any;
  data?: any;
}
...
export class GameState {
  ...
  setInitialHands(hands: Card[][]): GameState {
    const newState = this.clone();
    newState._initialHands = hands.map(h => [...h]) as readonly Card[][];
    return newState;
  }
  ...
  toSnapshot(): GameStateSnapshot {
    return {
      ...
      rounds: this._rounds.map(r => r.toSnapshot()),
  ...
  static fromSnapshot(snapshot: GameStateSnapshot, config: GameConfig): GameState {
    ...
    state._rounds = Object.freeze(snapshot.rounds.map(r => RoundData.fromSnapshot(r)));
