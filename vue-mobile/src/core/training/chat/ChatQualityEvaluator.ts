...
export class ChatQualityEvaluator {
  ...
  constructor(config: ChatQualityEvaluatorConfig) {
    ...
    // 确保autoEvaluation有默认值
    if (this.config.autoEvaluation === undefined) {
      this.config.autoEvaluation = true;
    }
  }
  ...
  async evaluate(sample: ChatTrainingSample): Promise<ChatQualityMetrics> {
    ...
    if (this.config.llmEvaluation?.enabled) {
      try {
        ...
        // 综合自动评估和LLM评估
        metrics.relevance = (metrics.relevance + (llmMetrics.relevance ?? metrics.relevance)) / 2;
        metrics.engagement = (metrics.engagement + (llmMetrics.engagement ?? metrics.engagement)) / 2;
        metrics.appropriateness = (metrics.appropriateness + (llmMetrics.appropriateness ?? metrics.appropriateness)) / 2;
      } catch (error) {
      }
    }
