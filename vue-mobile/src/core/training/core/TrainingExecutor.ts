...
export class TrainingExecutor {
  ...
  async initialize(config: TrainingConfig): Promise<void> {
    ...
    if (config.type === 'chat' || config.type === 'hybrid') {
      ...
      // ChatTrainer需要ChatQualityEvaluator
      const { ChatQualityEvaluator } = await import('../chat/ChatQualityEvaluator');
      const evaluator = new ChatQualityEvaluator({ autoEvaluation: true });
      this.chatTrainer = new ChatTrainer({ qualityEvaluator: evaluator });
    }
  }
