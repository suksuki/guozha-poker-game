...
describe('GameStore - "不要"功能', () => {
  ...
  describe('首家限制', () => {
    ...
    it('首家必须出牌', async () => {
      ...
      // 首家不能不要
      const passResult = await store.pass();
      expect(passResult.success).toBe(false);
      
      // 但可以出牌
      const playResult = await store.playCards([humanPlayer.hand[0]]);
  ...
  describe('游戏状态检查', () => {
    it('游戏未开始时不能不要', async () => {
      ...
      expect(result.success).toBe(false);
      // 游戏未开始时可能返回"游戏未开始"或"当前回合不存在"
      expect(result.message).toMatch(/游戏未开始|当前回合不存在/);
    });
  });
});
