# 在 192.168.0.13 上启动 MeLo TTS 服务

## 📋 准备工作

### 1. 确认当前位置

你现在在：`~/melotts/MeloTTS` 目录（在 192.168.0.13 服务器上）

### 2. 检查依赖

```bash
# 检查 MeLo TTS 是否已安装
python3 -c "from melo.api import TTS; print('✅ MeLo TTS 已安装')"

# 如果报错，安装 MeLo TTS
pip install git+https://github.com/myshell-ai/MeloTTS.git

# 安装 API 服务器依赖
pip install fastapi uvicorn pydantic
```

## 🚀 启动步骤

### 方法一：使用项目提供的脚本（推荐）

#### 第 1 步：复制启动脚本到 0.13 服务器

在 **192.168.0.13 服务器**上执行：

```bash
# 进入你的 MeLo TTS 目录
cd ~/melotts/MeloTTS

# 从游戏项目复制脚本
cp /Ubuntu/home/jin/guozha_poker_game/start-melo-tts-server.py .

# 或者使用 scp 从另一台机器复制
# scp /path/to/guozha_poker_game/start-melo-tts-server.py hlsystem@192.168.0.13:~/melotts/MeloTTS/
```

#### 第 2 步：启动服务

```bash
python3 start-melo-tts-server.py
```

你会看到：

```
============================================================
🎤 MeLo TTS API 服务器
============================================================

📡 监听地址: http://0.0.0.0:7860
🏠 本地访问: http://localhost:7860
🌐 网络访问: http://192.168.0.13:7860

📚 API 文档: http://localhost:7860/docs
❤️  健康检查: http://localhost:7860/health

============================================================
按 Ctrl+C 停止服务器
============================================================

INFO:     Started server process
INFO:     Uvicorn running on http://0.0.0.0:7860
🔄 正在加载 MeLo TTS 模型...
✅ MeLo TTS 模型加载成功！
```

### 方法二：手动创建脚本

如果无法复制文件，可以手动创建：

```bash
# 在 ~/melotts/MeloTTS 目录下
nano tts-server.py
```

粘贴以下内容：

```python
#!/usr/bin/env python3
from fastapi import FastAPI, HTTPException
from fastapi.responses import Response
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel
import uvicorn, logging, io

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

app = FastAPI()
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

_tts_model = None

def get_tts_model():
    global _tts_model
    if not _tts_model:
        from melo.api import TTS
        logger.info("加载模型...")
        _tts_model = TTS(language='ZH', device='auto')
        logger.info("✅ 模型加载完成")
    return _tts_model

class TTSRequest(BaseModel):
    text: str
    lang: str = "ZH"

@app.get("/health")
def health():
    get_tts_model()
    return {"status": "ok", "service": "Melo TTS"}

@app.post("/tts")
def tts(req: TTSRequest):
    if not req.text:
        raise HTTPException(400, "文本不能为空")
    
    model = get_tts_model()
    sid = model.hps.data.spk2id.get(req.lang, list(model.hps.data.spk2id.values())[0])
    
    logger.info(f"合成: {req.text[:30]}...")
    
    out = io.BytesIO()
    model.tts_to_file(req.text, sid, out, format='wav', speed=1.0)
    
    logger.info(f"✅ 成功")
    return Response(content=out.getvalue(), media_type="audio/wav")

if __name__ == "__main__":
    print("启动 MeLo TTS 服务器: http://0.0.0.0:7860")
    uvicorn.run(app, host="0.0.0.0", port=7860)
```

保存（Ctrl+X, Y, Enter）并启动：

```bash
python3 tts-server.py
```

## ✅ 测试服务

### 测试 1：本地健康检查

在 **0.13 服务器**上，打开新的终端窗口：

```bash
curl http://localhost:7860/health
```

应该返回：
```json
{"status":"ok","service":"Melo TTS"}
```

### 测试 2：本地 TTS 功能

```bash
curl -X POST http://localhost:7860/tts \
  -H "Content-Type: application/json" \
  -d '{"text": "你好世界", "lang": "ZH"}' \
  --output test.wav

# 检查生成的文件
ls -lh test.wav
```

### 测试 3：从客户端机器测试

在 **APP 客户端机器**上：

```bash
# 健康检查
curl http://192.168.0.13:7860/health

# TTS 测试
curl -X POST http://192.168.0.13:7860/tts \
  -H "Content-Type: application/json" \
  -d '{"text": "远程测试", "lang": "ZH"}' \
  --output remote-test.wav
```

如果失败，检查防火墙：

```bash
# Ubuntu/Debian
sudo ufw allow 7860/tcp
sudo ufw status

# CentOS/RHEL
sudo firewall-cmd --add-port=7860/tcp --permanent
sudo firewall-cmd --reload
```

## 🔧 后台运行（可选）

如果你想让服务在后台运行，关闭终端后仍然运行：

### 方法 1：使用 nohup

```bash
nohup python3 start-melo-tts-server.py > tts.log 2>&1 &

# 查看日志
tail -f tts.log

# 停止服务
ps aux | grep start-melo-tts-server
kill <PID>
```

### 方法 2：使用 screen（推荐）

```bash
# 创建新的 screen 会话
screen -S melotts

# 启动服务
python3 start-melo-tts-server.py

# 按 Ctrl+A, 然后按 D 分离会话（服务继续运行）

# 重新连接查看
screen -r melotts

# 列出所有会话
screen -ls
```

### 方法 3：使用 tmux

```bash
# 创建新会话
tmux new -s melotts

# 启动服务
python3 start-melo-tts-server.py

# 按 Ctrl+B, 然后按 D 分离

# 重新连接
tmux attach -t melotts
```

## 📊 查看服务状态

### 查看进程

```bash
ps aux | grep python | grep tts
```

### 查看端口

```bash
netstat -tlnp | grep 7860
# 或
ss -tlnp | grep 7860
```

### 查看日志

如果使用 nohup：
```bash
tail -f tts.log
```

### 访问 API 文档

浏览器打开：`http://192.168.0.13:7860/docs`

## ❌ 常见问题

### Q1: ModuleNotFoundError: No module named 'melo'

```bash
pip install git+https://github.com/myshell-ai/MeloTTS.git
```

### Q2: ModuleNotFoundError: No module named 'fastapi'

```bash
pip install fastapi uvicorn pydantic
```

### Q3: 端口 7860 已被占用

```bash
# 查找占用端口的进程
lsof -i :7860

# 停止该进程
kill <PID>

# 或修改端口（在脚本最后一行）
uvicorn.run(app, host="0.0.0.0", port=8860)  # 改为 8860
```

### Q4: 客户端无法连接

**检查清单：**

1. 服务器是否在运行？
   ```bash
   curl http://localhost:7860/health
   ```

2. 防火墙是否开放？
   ```bash
   sudo ufw status
   ```

3. 网络是否连通？
   ```bash
   ping 192.168.0.13
   ```

## ✅ 验证清单

启动成功的标志：

- [ ] 服务器输出显示 "✅ MeLo TTS 模型加载成功！"
- [ ] 服务器显示 "Uvicorn running on http://0.0.0.0:7860"
- [ ] 本地健康检查通过：`curl http://localhost:7860/health`
- [ ] 本地 TTS 测试生成了音频文件
- [ ] 远程健康检查通过：`curl http://192.168.0.13:7860/health`

## 🎉 成功！

服务启动后，你会看到：

```
============================================================
🎤 MeLo TTS API 服务器
============================================================

📡 监听地址: http://0.0.0.0:7860
🏠 本地访问: http://localhost:7860
🌐 网络访问: http://192.168.0.13:7860

============================================================

INFO:     Uvicorn running on http://0.0.0.0:7860
✅ MeLo TTS 模型加载成功！
```

## 📝 下一步

服务启动成功后，配置游戏 APP 连接到这个服务器：

**参考文档**：`docs/setup/melo-tts-remote-connection.md`

**快速配置**：在 `src/App.tsx` 中添加：

```typescript
meloConfig: {
  baseUrl: 'http://192.168.0.13:7860'
}
```

