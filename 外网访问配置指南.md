# 🌐 外网访问 0.13 服务器的 MeLo TTS 和 Ollama 配置指南

## 📋 当前服务状态

你在 **192.168.0.13** 服务器上运行了以下服务：

| 服务 | 端口 | 内网访问地址 | 绑定地址 |
|------|------|------------|---------|
| MeLo TTS | 7860 | http://192.168.0.13:7860 | 0.0.0.0 ✅ |
| Ollama | 11434 | http://192.168.0.13:11434 | localhost ⚠️ |

## 🎯 外网访问方案

### 方案一：内网穿透（推荐，最简单）

使用内网穿透工具可以让外网直接访问内网服务，无需公网 IP 或路由器配置。

#### 1. 使用 frp（免费，推荐）

**在 0.13 服务器上安装 frp 客户端：**

```bash
# 下载 frp
cd ~
wget https://github.com/fatedier/frp/releases/download/v0.52.3/frp_0.52.3_linux_amd64.tar.gz
tar -xzf frp_0.52.3_linux_amd64.tar.gz
cd frp_0.52.3_linux_amd64

# 创建配置文件
cat > frpc.ini << 'EOF'
[common]
server_addr = frp服务器地址
server_port = 7000
token = your_token

[melotts]
type = tcp
local_ip = 127.0.0.1
local_port = 7860
remote_port = 17860

[ollama]
type = tcp
local_ip = 127.0.0.1
local_port = 11434
remote_port = 11434
EOF

# 启动 frp 客户端
./frpc -c frpc.ini
```

**外网访问地址：**
- MeLo TTS: `http://frp服务器IP:17860`
- Ollama: `http://frp服务器IP:11434`

#### 2. 使用 ngrok（简单但需要注册）

```bash
# 安装 ngrok
curl -s https://ngrok-agent.s3.amazonaws.com/ngrok.asc | \
  sudo tee /etc/apt/trusted.gpg.d/ngrok.asc >/dev/null && \
  echo "deb https://ngrok-agent.s3.amazonaws.com buster main" | \
  sudo tee /etc/apt/sources.list.d/ngrok.list && \
  sudo apt update && sudo apt install ngrok

# 认证（需要在 ngrok.com 注册获取 token）
ngrok config add-authtoken YOUR_TOKEN

# 同时暴露两个端口
ngrok http 7860 &
ngrok http 11434 &
```

#### 3. 使用 CloudFlare Tunnel（免费，专业）

```bash
# 安装 cloudflared
wget https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64
chmod +x cloudflared-linux-amd64
sudo mv cloudflared-linux-amd64 /usr/local/bin/cloudflared

# 登录 CloudFlare
cloudflared tunnel login

# 创建隧道
cloudflared tunnel create poker-services

# 配置隧道
cat > ~/.cloudflared/config.yml << 'EOF'
url: http://localhost:7860
tunnel: <tunnel-id>
credentials-file: /home/your-user/.cloudflared/<tunnel-id>.json
ingress:
  - hostname: melotts.your-domain.com
    service: http://localhost:7860
  - hostname: ollama.your-domain.com
    service: http://localhost:11434
  - service: http_status:404
EOF

# 运行隧道
cloudflared tunnel run poker-services
```

---

### 方案二：路由器端口转发（需要公网 IP）

如果你有公网 IP 或者能访问路由器管理界面：

#### 步骤 1：配置路由器端口转发

登录路由器管理界面（通常是 192.168.0.1 或 192.168.1.1），找到"端口转发"或"虚拟服务器"设置：

| 服务名 | 外部端口 | 内部 IP | 内部端口 | 协议 |
|--------|----------|---------|----------|------|
| MeLo-TTS | 7860 | 192.168.0.13 | 7860 | TCP |
| Ollama | 11434 | 192.168.0.13 | 11434 | TCP |

#### 步骤 2：确保 Ollama 绑定到 0.0.0.0

Ollama 默认只监听 localhost，需要修改：

```bash
# 在 0.13 服务器上
# 停止当前的 Ollama 服务
pkill ollama

# 设置环境变量并启动
export OLLAMA_HOST=0.0.0.0:11434
ollama serve &

# 或者创建系统服务
sudo tee /etc/systemd/system/ollama.service > /dev/null << 'EOF'
[Unit]
Description=Ollama LLM Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
Environment="OLLAMA_HOST=0.0.0.0:11434"
ExecStart=/usr/local/bin/ollama serve
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
EOF

# 启动服务
sudo systemctl daemon-reload
sudo systemctl enable ollama
sudo systemctl start ollama
```

#### 步骤 3：配置防火墙

```bash
# Ubuntu/Debian
sudo ufw allow 7860/tcp comment "MeLo TTS"
sudo ufw allow 11434/tcp comment "Ollama"
sudo ufw reload

# CentOS/RHEL
sudo firewall-cmd --permanent --add-port=7860/tcp
sudo firewall-cmd --permanent --add-port=11434/tcp
sudo firewall-cmd --reload
```

#### 步骤 4：获取公网 IP

```bash
# 查看公网 IP
curl ifconfig.me
# 或
curl ipinfo.io/ip
```

**外网访问地址：**
- MeLo TTS: `http://你的公网IP:7860`
- Ollama: `http://你的公网IP:11434`

---

### 方案三：使用 VPN（最安全）

如果是团队内部使用，建议搭建 VPN：

#### 使用 WireGuard（推荐）

**在 0.13 服务器上安装 WireGuard：**

```bash
# 安装
sudo apt update
sudo apt install wireguard

# 生成密钥
wg genkey | tee privatekey | wg pubkey > publickey

# 配置服务器
sudo tee /etc/wireguard/wg0.conf > /dev/null << 'EOF'
[Interface]
PrivateKey = 服务器私钥
Address = 10.0.0.1/24
ListenPort = 51820
PostUp = iptables -A FORWARD -i wg0 -j ACCEPT
PostDown = iptables -D FORWARD -i wg0 -j ACCEPT

[Peer]
PublicKey = 客户端公钥
AllowedIPs = 10.0.0.2/32
EOF

# 启动 WireGuard
sudo wg-quick up wg0
sudo systemctl enable wg-quick@wg0
```

**在客户端配置：**

```ini
[Interface]
PrivateKey = 客户端私钥
Address = 10.0.0.2/24
DNS = 8.8.8.8

[Peer]
PublicKey = 服务器公钥
Endpoint = 服务器公网IP:51820
AllowedIPs = 192.168.0.0/24
PersistentKeepalive = 25
```

连接 VPN 后，直接使用内网地址：
- MeLo TTS: `http://192.168.0.13:7860`
- Ollama: `http://192.168.0.13:11434`

---

## 🔒 安全建议

### 1. 添加认证（推荐）

修改 MeLo TTS 服务，添加 API Key 认证：

```python
# 在 start-melo-tts-server.py 中添加
from fastapi import Header, HTTPException

API_KEY = "your-secret-api-key"  # 设置你的密钥

async def verify_api_key(x_api_key: str = Header()):
    if x_api_key != API_KEY:
        raise HTTPException(status_code=401, detail="无效的 API Key")
    return x_api_key

# 在路由中添加依赖
@app.post("/tts", dependencies=[Depends(verify_api_key)])
async def synthesize_speech(request: TTSRequest):
    # ... 原有代码
```

### 2. 使用反向代理（Nginx）

```bash
# 安装 Nginx
sudo apt install nginx

# 配置反向代理
sudo tee /etc/nginx/sites-available/poker-services > /dev/null << 'EOF'
server {
    listen 80;
    server_name your-domain.com;

    # MeLo TTS
    location /tts/ {
        proxy_pass http://localhost:7860/;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }

    # Ollama
    location /ollama/ {
        proxy_pass http://localhost:11434/;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }

    # 限制访问
    # allow 你的IP地址;
    # deny all;
}
EOF

# 启用配置
sudo ln -s /etc/nginx/sites-available/poker-services /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl reload nginx
```

### 3. 配置 HTTPS（使用 Let's Encrypt）

```bash
# 安装 certbot
sudo apt install certbot python3-certbot-nginx

# 获取证书
sudo certbot --nginx -d your-domain.com

# 自动续期
sudo certbot renew --dry-run
```

---

## ✅ 测试外网访问

### 1. 测试 MeLo TTS

```bash
# 使用外网地址测试
curl -X POST http://你的外网地址:7860/tts \
  -H "Content-Type: application/json" \
  -d '{"text": "外网测试成功", "lang": "ZH"}' \
  --output test-external.wav

# 检查文件
ls -lh test-external.wav
```

### 2. 测试 Ollama

```bash
# 健康检查
curl http://你的外网地址:11434/api/tags

# 测试对话
curl -X POST http://你的外网地址:11434/api/chat \
  -H "Content-Type: application/json" \
  -d '{
    "model": "qwen2:0.5b",
    "messages": [
      {"role": "user", "content": "你好"}
    ],
    "stream": false
  }'
```

---

## 📊 监控和维护

### 1. 设置后台运行

创建启动脚本：

```bash
#!/bin/bash
# ~/start-all-services.sh

# 启动 MeLo TTS
cd ~/melotts/MeloTTS
nohup python3 start-melo-tts-server.py > logs/melotts.log 2>&1 &

# 启动 Ollama
export OLLAMA_HOST=0.0.0.0:11434
nohup ollama serve > logs/ollama.log 2>&1 &

echo "所有服务已启动"
```

### 2. 使用 systemd 管理服务

**MeLo TTS 服务：**

```bash
sudo tee /etc/systemd/system/melotts.service > /dev/null << 'EOF'
[Unit]
Description=MeLo TTS API Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
WorkingDirectory=/home/YOUR_USERNAME/melotts/MeloTTS
ExecStart=/usr/bin/python3 /home/YOUR_USERNAME/melotts/MeloTTS/start-melo-tts-server.py
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
EOF

sudo systemctl daemon-reload
sudo systemctl enable melotts
sudo systemctl start melotts
```

### 3. 监控服务状态

```bash
# 检查服务状态
sudo systemctl status melotts
sudo systemctl status ollama

# 查看日志
sudo journalctl -u melotts -f
sudo journalctl -u ollama -f

# 检查端口
ss -tlnp | grep -E '7860|11434'
```

---

## 🎉 推荐方案总结

### 适合个人/临时使用：
✅ **ngrok** - 最简单，5分钟搞定

### 适合长期使用：
✅ **frp + 自己的服务器** - 稳定可靠
✅ **CloudFlare Tunnel** - 免费且专业

### 适合团队内部：
✅ **WireGuard VPN** - 最安全

### 如果有公网 IP：
✅ **路由器端口转发 + Nginx反向代理** - 最直接

---

## 📝 配置客户端应用

外网访问配置成功后，在你的游戏应用中修改配置：

**编辑 `src/App.tsx` 或配置文件：**

```typescript
// 使用外网地址
const config = {
  meloTTS: {
    baseUrl: 'http://你的外网地址:7860'  // 或使用域名
  },
  ollama: {
    baseUrl: 'http://你的外网地址:11434',
    model: 'qwen2:0.5b'
  }
};
```

---

## ❓ 常见问题

### Q1: 端口转发配置后仍无法访问？

检查清单：
1. ✅ 服务是否绑定到 0.0.0.0？
2. ✅ 防火墙是否开放端口？
3. ✅ 路由器端口转发是否正确？
4. ✅ 是否有多层路由器？（光猫需要桥接模式）

### Q2: 如何查看是否有公网 IP？

```bash
# 查看公网 IP
curl ifconfig.me

# 在路由器管理界面查看 WAN IP
# 如果是 10.x.x.x 或 100.x.x.x，则不是公网 IP
```

### Q3: 没有公网 IP 怎么办？

- 联系运营商申请公网 IP（可能需要额外费用）
- 使用内网穿透工具（frp、ngrok、CloudFlare Tunnel）
- 租用云服务器中转

### Q4: 如何确保服务安全？

1. 添加 API Key 认证
2. 使用 HTTPS
3. 配置 Nginx 限制访问 IP
4. 使用 VPN
5. 定期更新服务

---

需要我帮你配置哪个方案吗？😊

